...
// weeks. Paying ahead is expected: 7 days means Mon–Fri of this week plus
// Mon–Tue of the next week — weekends are never counted. Only days that don't
// already have savings are filled, so a week that was partially covered by an
// earlier advance payment is completed first before moving on.
//
// A day is ALWAYS worth the member's daily rate. The number of days a payment
// covers comes from the amount (₦2100 at ₦300/day = 7 days), never from a
...
// Pure preview of which days an amount would cover, starting from the earliest
// unpaid day of the given week and rolling forward. Used by the admin mark-paid
// dialog so the exact covered days are visible before confirming.
export function planDayCoverage(
  state: ThriftState,
  memberId: string,
  startWeekId: string,
  amount: number
): { week: ThriftWeek; dates: string[] }[] {
  if (amount <= 0) return [];
  const startIndex = state.weeks.findIndex((w) => w.id === startWeekId);
  if (startIndex < 0) return [];
  const plan = getPlanForWeek(state, memberId, state.weeks[startIndex]);
  const daily = plan.dailyAmount || 1;
  const totalDays = Math.max(1, Math.round(amount / daily));
  const alreadySaved = new Set(
    state.savings.filter((s) => s.memberId === memberId).map((s) => s.date)
  );
  const result: { week: ThriftWeek; dates: string[] }[] = [];
  let remaining = totalDays;
  let i = startIndex;
  while (remaining > 0 && i < state.weeks.length) {
    const week = state.weeks[i];
    const missing = week.days.filter((d) => !alreadySaved.has(d.date));
    const take = Math.min(remaining, missing.length);
    if (take > 0) {
      const dates = missing.slice(0, take).map((d) => d.date);
      result.push({ week, dates });
      for (const d of dates) alreadySaved.add(d);
      remaining -= take;
    }
    i += 1;
  }
